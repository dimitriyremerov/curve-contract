# @version ^0.2.12
"""
@title StableSwap Unslashed Finance Basket Collateral Token / stETH-ETH LP
@author Curve.Fi, Unslashed Finance
@license Copyright (c) Curve.Fi, 2020 - all rights reserved
@notice Based on Metapool template
"""
...
@view
@internal
def _get_D_mem(rates: uint256[N_COINS], _balances: uint256[N_COINS], _amp: uint256) -> uint256:
    return self._get_D(self._xp_mem(rates, _balances), _amp)
...
def get_virtual_price() -> uint256:
    ...
    xp: uint256[N_COINS] = self._xp(self._rates(vp_rate))
...
def calc_token_amount(_amounts: uint256[N_COINS], _is_deposit: bool) -> uint256:
    ...
    """
    amp: uint256 = self._A()
    rates: uint256[N_COINS] = self._rates(self._vp_rate_ro())
...
def add_liquidity(_amounts: uint256[N_COINS], _min_mint_amount: uint256) -> uint256:
    ...
    amp: uint256 = self._A()
    rates: uint256[N_COINS] = self._rates(self._vp_rate())
...
def get_dy_underlying(i: int128, j: int128, _dx: uint256) -> uint256:
    ...
    vp_rate: uint256 = self._vp_rate_ro()
    rates: uint256[N_COINS] = self._rates(vp_rate)
    xp: uint256[N_COINS] = self._xp(rates)
...
def _calc_withdraw_one_coin(_token_amount: uint256, i: int128, _vp_rate: uint256) -> (uint256, uint256, uint256):
    ...
    amp: uint256 = self._A()
    rates: uint256[N_COINS] = self._rates(_vp_rate)
    xp: uint256[N_COINS] = self._xp(rates)
